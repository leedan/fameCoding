package cn.famecode.ai.infrastructure.security;
...
public class PasswordEncryptor {
    ...
    /** 默认密钥（生产环境务必通过环境变量 FAMECODE_SECRET_KEY 覆盖） */
    ...
    public PasswordEncryptor(String secretKey) {
        ...
        if (key == null || key.isBlank()) {
            key = System.getenv("FAMECODE_SECRET_KEY");
        }
        if (key == null || key.isBlank()) {
            key = DEFAULT_SECRET_KEY;
            log.warn("未配置 FAMECODE_SECRET_KEY 环境变量，使用默认密钥，生产环境请务必配置！");
        }
