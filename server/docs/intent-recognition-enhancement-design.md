...
> 基于 FameCode 上下文记忆与意图识别架构，适配 WaLiSSH 的 Spring Boot DDD + Google ADK 技术栈。
...
```
walissh-server-domain/src/main/java/cn/famecode/ai/domain/agent/
├── service/
...
```java
package cn.famecode.ai.domain.agent.model.valobj.prompt;
...
```java
package cn.famecode.ai.domain.agent.service.prompt;
...
```java
package cn.famecode.ai.domain.agent.service.prompt.dynamic;
...
采用 FameCode 的 **Provider-Reducer 管道模式**，将上下文收集与消息裁剪解耦。Provider 负责收集各维度上下文，Reducer 负责在 token 预算内裁剪消息。

### 4.2 ContextProvider 接口

```java
package cn.famecode.ai.domain.agent.service.context.provider;
...
```java
package cn.famecode.ai.domain.agent.service.context.reducer;
...
```java
package cn.famecode.ai.domain.agent.service.context;
...
采用 **两层分类器链**（规则 + LLM），适配后端服务场景（省去 FameCode 前端的"模型分类器"层，直接规则 + LLM 两层，降低延迟）。
...
```java
package cn.famecode.ai.domain.agent.service.enhance;

import cn.famecode.ai.domain.agent.service.IIntentEnhancerService;
...
## 九、与 FameCode 设计的关键适配差异

| 维度 | FameCode | WaLiSSH 适配 |
|------|----------|-------------|
