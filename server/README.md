# FameCode Server - ai coding 辅助编码
...
├── famecode-server-app/             # 启动模块（配置、入口、Agent YAML）
├── famecode-server-api/             # 对外接口定义
├── famecode-server-domain/          # 领域层（Agent 装配、对话、意图识别）
├── famecode-server-infrastructure/  # 基础设施层（持久化、外部适配）
├── famecode-server-trigger/         # 触发器层（HTTP Controller、Job）
├── famecode-server-case/            # 用例层（ReAct 执行引擎、SSE 流式输出）
└── famecode-server-types/           # 通用类型定义
```
...
Agent YAML 定义了每个智能体使用的大模型。文件位于 `famecode-server-app/src/main/resources/agent/` 下：
...
编辑 `famecode-server-app/src/main/resources/application-dev.yml`：
...
# 启动应用
cd famecode-server-app
mvn spring-boot:run
```

或直接运行主类 `cn.famecode.ai.Application`。
