package cn.famecode.ai.cases.react.node;

import cn.famecode.ai.api.dto.ChatRequestDTO;
import cn.famecode.ai.api.dto.ReActResultDTO;
import cn.famecode.ai.cases.react.AbstractAIAgentReActSupport;
import cn.famecode.ai.cases.react.factory.DefaultReActFactory;
import cn.famecode.ai.domain.agent.model.valobj.AiAgentRegisterVO;
import cn.famecode.ai.domain.agent.service.IPromptService;
import cn.famecode.ai.domain.agent.service.IChatContextService;
import cn.famecode.ai.domain.agent.service.IIntentService;
import cn.famecode.ai.domain.agent.model.valobj.intent.IntentResultVO;
import cn.famecode.ai.domain.agent.model.valobj.intent.IntentTypeEnumVO;
import cn.famecode.ai.domain.agent.service.intent.enhancer.IntentOrchestrator;
...
import cn.famecode.ai.domain.agent.service.armory.matter.mcp.server.SshExecuteMcpService;
import cn.famecode.ai.domain.agent.service.armory.matter.tools.SshExecuteAdkTool;
import cn.famecode.ai.domain.agent.service.armory.matter.tools.SubAgentAdkTool;
...
public class AiCallNode extends AbstractAIAgentReActSupport {
    ...
    protected ReActResultDTO doApply(ChatRequestDTO requestParameter, DefaultReActFactory.DynamicContext dynamicContext) throws Exception {
        ...
        try {
            ...
            cn.famecode.ai.cases.react.notifier.SseToolProgressNotifier.bind(
                    dynamicContext.getSessionId(), emitter);
            // 绑定指令分发器（本地指令通过 SSE 下发到 Client）
            cn.famecode.ai.domain.agent.service.armory.matter.tools.command.CommandDispatcher.bindEmitter(
                    dynamicContext.getSessionId(), emitter);
                ...
                try {
                    ...
                    while (events.hasNext()) {
                        ...
                        if (now - lastHeartbeatTime > HEARTBEAT_INTERVAL_MS) {
                            ...
                            boolean heartbeatOk = cn.famecode.ai.domain.agent.service.armory.matter.tools.command.CommandDispatcher.sendHeartbeat();
                            if (!heartbeatOk) {
                    ...
                    String serverErrorDetail = extractWebClientErrorDetail(e);
                    if (serverErrorDetail != null && !serverErrorDetail.isEmpty()) {
                        log.error("【大模型服务端返回 4xx/5xx 错误详情】: {}", serverErrorDetail);
                        errorBuilder.append("大模型服务错误: ").append(serverErrorDetail);
                    } else {
            ...
            // 解绑 SSE 进度通知器
            cn.famecode.ai.cases.react.notifier.SseToolProgressNotifier.unbind(dynamicContext.getSessionId());
            // 解绑指令分发器
            cn.famecode.ai.domain.agent.service.armory.matter.tools.command.CommandDispatcher.unbindEmitter(dynamicContext.getSessionId());
    ...
    private String buildEnrichedMessage(String userMessage, DefaultReActFactory.DynamicContext dynamicContext) {
        ...
        cn.famecode.ai.api.dto.ProjectContextDTO projectCtxForIntent = dynamicContext.getProjectContext();
        ...
        // 委托领域服务构建富化消息
        cn.famecode.ai.api.dto.ProjectContextDTO projectCtx = dynamicContext.getProjectContext();
    ...
    private String extractWebClientErrorDetail(Throwable t) {
        Throwable curr = t;
        while (curr != null) {
            if (curr instanceof org.springframework.web.reactive.function.client.WebClientResponseException wcre) {
                return String.format("HTTP %s from %s - %s",
                        wcre.getStatusCode(),
                        wcre.getRequest() != null ? wcre.getRequest().getURI() : "unknown",
