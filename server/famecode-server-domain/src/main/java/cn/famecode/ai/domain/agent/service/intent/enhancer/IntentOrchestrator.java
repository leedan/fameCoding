package cn.famecode.ai.domain.agent.service.intent.enhancer;
 ...
 * <p>
 * 对标 FameCode 的 IntentService 完整流程：
 * 1. 规则快速路径 → 高置信度直接返回
 * 2. 异步模型预分析 → LLM 分类
 * 3. 信号提取 → 结构化信息
 * 4. 指代消解 → "它"/"这个" → 具体实体
...
public class IntentOrchestrator {
    ...
    public static class EnhancedIntentResult {
        private cn.famecode.ai.domain.agent.model.valobj.intent.IntentTypeEnumVO intent;
