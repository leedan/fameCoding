...
import cn.famecode.ai.domain.agent.service.util.OpenAiUrlNormalizer;
...
public class LLMIntentClassifier implements IIntentClassifier {
    ...
    @Value("${intent-ai-api.completions-path:v1/chat/completions}")
    ...
    @PostConstruct
    public void init() {
        OpenAiUrlNormalizer.NormalizedUrl normalized = OpenAiUrlNormalizer.normalize(apiHost, completionsPath);
        log.info("LLMIntentClassifier 初始化: rawHost={}, rawPath={}, normalizedBaseUrl={}, normalizedCompletionsPath={}, model={}",
                apiHost, completionsPath, normalized.getBaseUrl(), normalized.getCompletionsPath(), model);

        String effectiveApiKey = ("EMPTY".equalsIgnoreCase(apiKey) || "none".equalsIgnoreCase(apiKey)) ? "" : apiKey;
